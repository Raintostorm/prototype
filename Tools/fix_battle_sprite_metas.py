#!/usr/bin/env python3
"""Generate + fix sprite metas for Resources/UI/Battle."""
import re
import struct
import uuid
...
DEFAULT_RESOURCE = ROOT / "Assets/SpinSquad/Resources/UI/Battle"
...
def main():
    ...
    resource = Path(sys.argv[1]).resolve() if len(sys.argv) > 1 else DEFAULT_RESOURCE
    if not resource.is_dir():
        print(f"No folder {resource} — run sync script first.")
        return
    n = 0
    for png in sorted(resource.glob("*.png")):
        if fix_meta(png):
            n += 1
            print("fixed", png.name)
    print(f"Done. Updated {n} sprite metas in {resource.name}/.")
